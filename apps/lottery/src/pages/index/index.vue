<template>
  <AppLayout class="theme-lottery" :tabs="navTabs" :active-tab="activeTab" @tab-change="activeTab = $event">
        ...
        >
          ...
          <view class="game-header text-center mb-1 z-10 relative">
            ...
            <view class="flex justify-center mt-1">
              <text class="game-price-tag text-[10px] font-bold px-2 py-0.5 rounded-full bg-black/40 text-white/90 border border-white/10 uppercase tracking-widest">
                {{ game.priceDisplay }}
              </text>
            </view>
          </view>
          ...
          <view class="game-visual relative h-32 flex items-center justify-center my-3">
            ...
            <view class="pulsar absolute w-20 h-20 rounded-full blur-3xl opacity-60" :style="{ backgroundColor: game.color }" />
            
            <view class="relative z-10 flex flex-col items-center transform transition-all duration-300 group-hover:scale-110">
          ...
          <view class="game-stats mb-5 text-center z-10 relative mt-auto">
             <text class="block text-[10px] uppercase font-bold opacity-70 text-white tracking-[0.1em] mb-1">JACKPOT</text>
...
const { instantTypes, getLotteryType } = useLotteryTypes();
  ...
  const { address, chainType, invokeRead, getContractAddress, switchToAppChain } = useWallet();
...
.card-gold .buy-button { border-color: var(--lottery-buy-border-gold) !important; }
.card-platinum .buy-button { border-color: var(--lottery-buy-border-platinum) !important; }
.card-diamond .buy-button { border-color: var(--lottery-buy-border-diamond) !important; font-size: 14px; }
