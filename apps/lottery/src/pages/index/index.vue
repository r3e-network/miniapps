...
import { AppLayout, NeoDoc, NeoButton, NeoCard, NeoStats, ChainWarning } from "@shared/components";
...
const CARD_VARIANT_THRESHOLD = 5;

const getCardVariant = (price: number) => {
  if (price >= CARD_VARIANT_THRESHOLD) return "erobo-neo";
  return "erobo";
};

const getButtonVariant = (_price: number) => "primary";
...
const handleBuy = async (gameType: LotteryTypeInfo) => {
  ...
  try {
    ...
    console.error("[Lottery] Buy failed:", e);
  } finally {
