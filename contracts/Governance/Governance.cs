...
namespace NeoMiniAppPlatform.Contracts
{
    ...
    public class Governance : SmartContract
    {
        ...
        private static readonly byte[] PREFIX_USED_STAKE = new byte[] { 0x05 }; // Track used voting power
        ...
        public static void Vote(string proposalId, bool support, BigInteger amount)
        {
            ...
            BigInteger usedStake = GetUsedStake(voter);
            BigInteger availableStake = stake - usedStake;
            ExecutionEngine.Assert(availableStake >= amount, "insufficient available stake");
            ...
            // Track used voting power
            SetUsedStake(voter, usedStake + amount);
        ...
        private static BigInteger GetUsedStake(UInt160 voter)
        {
            StorageMap map = new StorageMap(Storage.CurrentContext, PREFIX_USED_STAKE);
            ByteString raw = map.Get(voter);
            return raw == null ? 0 : (BigInteger)raw;
        }

        private static void SetUsedStake(UInt160 voter, BigInteger amount)
        {
            StorageMap map = new StorageMap(Storage.CurrentContext, PREFIX_USED_STAKE);
            if (amount <= 0)
                map.Delete(voter);
            else
                map.Put(voter, amount);
        }

        /// <summary>
        /// Release voting power when proposal is finalized. Called by admin.
        /// </summary>
        public static void ReleaseVotingPower(string proposalId, UInt160 voter)
        {
            Proposal p = GetProposal(proposalId);
            ExecutionEngine.Assert(p.Finalized, "proposal not finalized");

            byte[] voteKey = Helper.Concat((byte[])ProposalKey(proposalId), (byte[])voter);
            ByteString voteAmount = VoteMap().Get(voteKey);
            if (voteAmount == null) return;

            BigInteger amount = (BigInteger)voteAmount;
            BigInteger usedStake = GetUsedStake(voter);
            BigInteger newUsedStake = usedStake > amount ? usedStake - amount : 0;
            SetUsedStake(voter, newUsedStake);
