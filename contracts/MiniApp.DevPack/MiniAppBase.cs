...
namespace NeoMiniAppPlatform.Contracts
{
    ...
    ///
    /// STORAGE LAYOUT:
    /// - 0x01-0x0A: Core (Admin, Gateway, PaymentHub, Pause, TimeLock with height check)
    /// - 0x0B-0x0F: Optional core (Automation, Badges, TotalUsers)
    ...
    public class MiniAppBase : SmartContract
    {
        ...
        // TimeLock storage (P0 security fix + TimeLock bypass fix)
        ...
        protected static readonly byte[] PREFIX_ADMIN_CHANGE_HEIGHT = new byte[] { 0x09 };
        protected static readonly byte[] PREFIX_TIMELOCK_DELAY = new byte[] { 0x0A };
        // Automation anchor (optional, for contracts needing periodic execution)
        protected static readonly byte[] PREFIX_AUTOMATION_ANCHOR = new byte[] { 0x0B };
        protected static readonly byte[] PREFIX_AUTOMATION_TASK = new byte[] { 0x0C };
        // Badge system (optional, for contracts with achievement tracking)
        protected static readonly byte[] PREFIX_USER_BADGES = new byte[] { 0x0D };
        protected static readonly byte[] PREFIX_USER_BADGE_COUNT = new byte[] { 0x0E };
        // Total users tracking (optional)
        protected static readonly byte[] PREFIX_TOTAL_USERS = new byte[] { 0x0F };
        ...
        private const long BLOCK_TIME_SECONDS = 15;                // ~15 seconds per block (Neo N3)
        private const long MIN_TIMELOCK_DELAY_BLOCKS = 240;        // Minimum blocks (1 hour = 240 blocks)
        ...
        public static BigInteger AdminChangeHeight() =>
            (BigInteger)Storage.Get(Storage.CurrentContext, PREFIX_ADMIN_CHANGE_HEIGHT);
        ...
        /// SECURITY FIX: Now stores both block height AND timestamp to prevent TimeLock bypass.
        /// Both conditions must be met before admin change executes.
        /// </summary>
        public static void ProposeAdmin(UInt160 newAdmin)
        {
            ...
            // Calculate delay in both seconds and blocks
            BigInteger delaySeconds = TimeLockDelay();
            BigInteger delayBlocks = delaySeconds / BLOCK_TIME_SECONDS;
            if (delayBlocks < MIN_TIMELOCK_DELAY_BLOCKS) delayBlocks = MIN_TIMELOCK_DELAY_BLOCKS;

            // Store both timestamp and block height
            BigInteger executeAfterTime = Runtime.Time + delaySeconds;
            BigInteger executeAfterHeight = Runtime.Height + delayBlocks;
        ...
        /// SECURITY FIX: Requires BOTH timestamp AND block height conditions to be met.
        /// This prevents miners from manipulating block timestamps to bypass TimeLock.
        /// </summary>
        public static void ExecuteAdminChange()
        {
            ...
            // SECURITY FIX: Check BOTH conditions to prevent TimeLock bypass
            // - Timestamp check: Prevents immediate execution
            // - Height check: Prevents miner timestamp manipulation
            // Even if a miner sets a future timestamp, they cannot produce blocks faster than the protocol rate
        ...
        /// <summary>
        /// Validates a payment receipt from PaymentHub and ATOMICALLY marks it as used.
        /// SECURITY FIX: This is now atomic - validates AND marks as used in a single call
        /// to prevent double-spend attacks. All MiniApps using this function are protected.
        /// </summary>
        protected static BigInteger ValidatePaymentReceipt(
            string appId, UInt160 payer, BigInteger minAmount, BigInteger receiptId)
        {
            ...
            // SECURITY: Mark as used IMMEDIATELY before any external calls (CEI pattern)
            // This prevents reentrancy and double-spend attacks
        ...
        /// <summary>
        /// [DEPRECATED] No longer needed - ValidatePaymentReceipt now atomically marks receipts.
        /// Kept for backward compatibility but logs a warning.
        /// </summary>
        [System.Obsolete("ValidatePaymentReceipt now atomically marks receipts. This method is no longer needed.")]
        protected static void MarkReceiptUsed(BigInteger receiptId)
        {
            // No-op: Receipt is already marked by ValidatePaymentReceipt
            // Kept for backward compatibility with any existing code that calls it
        }
