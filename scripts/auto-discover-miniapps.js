    ...
    if (fs.existsSync(configPath)) {
      ...
      entries.forEach((entry) => {
        if (entry?.app_id && entry?.address) {
          result[entry.app_id] = entry.address;
        }
      });
    }
  } catch (e) {
    console.warn(`  Warning: Could not parse ${label} for contract addresses`);
  }
  return result;
}

const contractAddressMapByChain = Object.entries(CONTRACT_CONFIGS).reduce((acc, [chainId, configPath]) => {
  acc[chainId] = loadContractAddresses(configPath, path.basename(configPath));
  return acc;
}, {});
...
function normalizePermissions(raw) {
  ...
  if (Array.isArray(raw)) {
    const tokens = raw.map((entry) => toString(entry).toLowerCase()).filter(Boolean);
    return {
      ...base,
      payments: tokens.includes("payments"),
      governance: tokens.includes("governance"),
      rng: tokens.includes("rng"),
      datafeed: tokens.includes("datafeed"),
      automation: tokens.includes("automation"),
      confidential: tokens.includes("confidential"),
    };
  }
  ...
  return {
    ...
    confidential: Boolean(permissions.confidential),
  };
}
...
function discoverMiniapp(appDir) {
  ...
  Object.entries(contractAddressMapByChain).forEach(([chainId, addressMap]) => {
    ...
    if (existingObj.address) return;
    rawContracts[chainId] = {
      ...existingObj,
      address,
    };
  });
